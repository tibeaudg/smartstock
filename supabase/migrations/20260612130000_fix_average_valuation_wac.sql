-- Moving weighted average (WAC) for inventory valuation — matches client-side stock movement ledger.
-- Also syncs products.purchase_price when correcting a receipt that matched the old default.

CREATE OR REPLACE FUNCTION public._stock_tx_valuation_direction(
...
DECLARE
    v_ref TEXT;
BEGIN
    v_ref := COALESCE(p_reference_number, '');

    -- isStockInTransaction (order matches src/lib/inventory/dashboardMetrics.ts)
    IF p_transaction_type IN ('incoming', 'in', 'purchase_order', 'scan_adjustment', 'return') THEN
        RETURN 'in';
    END IF;
    IF p_transaction_type = 'manual_adjustment' THEN
        IF position('_IN' in v_ref) > 0 OR v_ref NOT ILIKE '%OUT%' THEN
            RETURN 'in';
        END IF;
    END IF;

    -- isStockOutTransaction
    IF p_transaction_type IN ('outgoing', 'out', 'sales_order', 'damage') THEN
        RETURN 'out';
    END IF;
    IF p_transaction_type = 'manual_adjustment' AND v_ref ILIKE '%OUT%' THEN
        RETURN 'out';
    END IF;
    IF p_transaction_type = 'scan_adjustment' AND v_ref ILIKE '%OUTGOING%' THEN
        RETURN 'out';
    END IF;

    -- getTransactionDirection fallbacks (src/lib/inventory/stockMovementLedger.ts)
    IF p_transaction_type = 'cycle_count' THEN
        RETURN 'in';
    END IF;

...
-- quantity_in_stock may be TEXT in products table; cast to NUMERIC before COALESCE
...
BEGIN
    ...
    -- Check stock availability (cast quantity_in_stock first to avoid text/integer COALESCE mismatch)
    IF (COALESCE((v_product.quantity_in_stock)::NUMERIC, 0)) < p_quantity_fulfilled THEN
    ...
    -- Update product stock (cast quantity_in_stock first to avoid text/integer COALESCE mismatch)
    -- Result cast to TEXT to support products.quantity_in_stock when stored as TEXT
    UPDATE public.products
    SET quantity_in_stock = (COALESCE((quantity_in_stock)::NUMERIC, 0) - p_quantity_fulfilled)::TEXT,
        updated_at = NOW()
    WHERE id = COALESCE(v_so_item.variant_id, v_so_item.product_id);

-- Product health metrics for owner dashboard + per-user analytics consent

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS analytics_consent BOOLEAN;

COMMENT ON COLUMN public.profiles.analytics_consent IS
  'NULL = unknown; true = analytics allowed; false = user declined analytics cookies';
...
BEGIN
  ...
  SELECT jsonb_build_object(
        ...
        SELECT
          ...
          COUNT(*) FILTER (WHERE EXISTS (
            ...
            WHERE e.user_id = p.id
              ...
              AND e.timestamp::DATE BETWEEN p.created_at::DATE AND p.created_at::DATE + 7
    ...
    'import_failure_rate_7d', (
      SELECT CASE WHEN started = 0 THEN 0
...
GRANT EXECUTE ON FUNCTION public.get_product_health_summary(DATE, DATE) TO authenticated;

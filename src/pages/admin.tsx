...
function FeedbackTab() {
  ...
  const avgRating = feedbackList.length
    ? (feedbackList.filter(f => f.rating).reduce((sum, f) => sum + (f.rating ?? 0), 0) /
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-slate-400">No feedback submitted yet.</div>
      ) : (
...
export default function AdminPage() {
  ...
  const tabParam = searchParams.get('tab') as 'users' | 'notifications' | 'emails' | 'feedback' | null;
  const [activeTab, setActiveTabState] = useState<'users' | 'chats' | 'notifications' | 'emails' | 'feedback'>(
    tabParam && ['users', 'chats', 'notifications', 'emails', 'feedback'].includes(tabParam) ? tabParam : 'users'
  );

  const setActiveTab = (tab: 'users' | 'chats' | 'notifications' | 'emails' | 'feedback') => {
    setActiveTabState(tab);
    navigate(`/admin?tab=${tab}`, { replace: true });
  };

  useEffect(() => {
    if (tabParam && ['users', 'chats', 'notifications', 'emails', 'feedback'].includes(tabParam)) {

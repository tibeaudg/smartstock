...
export default function BulkImportPage() {
  ...
  const validateAndSetFile = async (selectedFile: File) => {
    ...
    try {
      ...
      if (!mapping.name) setShowMappingUI(true);
      else setShowMappingUI(false);
    ...
    >
      <div className="space-y-6">
        ...
        <div className="bg-gray-50 rounded-lg border border-gray-200 p-4 space-y-2">
          <label className="block text-sm font-medium text-gray-700">Upload Excel or CSV</label>
          {parseError && (
          ...
          <div
            ...
            className={`
              ...
              ${isParsing ? 'cursor-wait border-blue-400 bg-blue-50' : ''}
              ${!isParsing && isDragging ? 'border-blue-500 bg-blue-50' : ''}
              ${!isParsing && file && !isDragging ? 'border-green-500 bg-green-50' : ''}
              ${!isParsing && !file && !isDragging ? 'border-gray-300 hover:border-blue-400 hover:bg-blue-50' : ''}
            `}
          >
            <input
              ref={fileInputRef}
            ...
            {isParsing ? (
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-xs text-gray-400 mt-2">Supports Excel (.xlsx, .xls) and CSV (.csv), max 10MB</p>
              </div>
            )}
          </div>
        </div>

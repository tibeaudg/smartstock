...
async function handleCheckoutSessionCompleted(session: Stripe.Checkout.Session) {
  ...
  if (error) {
    ...
    await createInvoiceFromSubscription(subscription, moduleId, userId);
  }
}
...
async function handleInvoicePaymentSucceeded(invoice: Stripe.Invoice) {
  if (invoice.subscription) {
    ...
    await createInvoiceFromStripeInvoice(invoice);
  }
}
...
async function createInvoiceFromSubscription(subscription: Stripe.Subscription, moduleId: string, userId: string) {
  try {
    // Get module details
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        due_date: new Date(subscription.current_period_end * 1000).toISOString(),
      });
...
async function createInvoiceFromStripeInvoice(invoice: Stripe.Invoice) {
  try {
    // Get subscription details
    const subscription = await stripe.subscriptions.retrieve(invoice.subscription as string);
    const { moduleId, userId } = subscription.metadata || {};
    ...
    console.error('Error in createInvoiceFromStripeInvoice:', error);
  }
}

      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            {/* New Accounts Quick Access */}
            <div className="space-y-3 p-4 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-medium text-sm flex items-center gap-2 text-green-900">
                    <UserPlus className="w-4 h-4" />
                    New Accounts (Just Registered)
                  </h3>
                  <p className="text-xs text-green-700 mt-1">
                    Target users who just created their accounts - perfect for welcome emails sent right after registration
                  </p>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  onClick={() => {
                    setFormData({
                      ...formData,
                      name: formData.name || 'New Accounts (Last Hour)',
                      filters: {
                        ...formData.filters,
                        accountCreated: { operator: 'less_than', days: 0.042 }, // ~1 hour
                      },
                    });
                  }}
                ...
                <Button
                  ...
                  onClick={() => {
                    setFormData({
                      ...formData,
                      name: formData.name || 'New Accounts (Last 6 Hours)',
                      filters: {
                        ...formData.filters,
                        accountCreated: { operator: 'less_than', days: 0.25 }, // 6 hours
                      },
                    });
                  }}
                  className="bg-white hover:bg-green-100 border-green-300"
                ...
                <Button
                  ...
                  onClick={() => {
                    setFormData({
                      ...formData,
                      name: formData.name || 'New Accounts (Last 24 Hours)',
                      filters: {
                        ...formData.filters,
                        accountCreated: { operator: 'less_than', days: 1 },
                      },
                    });
                  }}
                  className="bg-white hover:bg-green-100 border-green-300"
                ...
                <Button
                  ...
                  onClick={() => {
                    setFormData({
                      ...formData,
                      name: formData.name || 'New Accounts (Last 3 Days)',
                      filters: {
                        ...formData.filters,
                        accountCreated: { operator: 'less_than', days: 3 },
                      },
                    });
                  }}
                  className="bg-white hover:bg-green-100 border-green-300"
                  ...
                  {availablePlans.length === 0 ? (
                    ...
                    availablePlans.map((plan) => (
                      <div key={plan} className="flex items-center space-x-2">
                        <Checkbox
                          id={`plan-${plan}`}
                          checked={formData.filters.plan.includes(plan)}
                  ...
                  {availableOrganizations.length === 0 ? (
                    ...
                    availableOrganizations.map((org) => (
                      <div key={org} className="flex items-center space-x-2">
                        <Checkbox
                          id={`org-${org}`}
                          checked={formData.filters.organization.includes(org)}
                          onCheckedChange={() => toggleFilter('organization', org)}
                        />
                        <Label htmlFor={`org-${org}`} className="text-sm font-normal cursor-pointer">
                          {org}
                        </Label>
                      </div>
                    ))
                ...
                <div className="space-y-2 p-3 border rounded-md">
                  {['user', 'admin', 'staff'].map((role) => (
                    <div key={role} className="flex items-center space-x-2">
                      <Checkbox
                        id={`role-${role}`}
                        checked={formData.filters.role.includes(role)}
            ...
            <div>
              ...
              <Select
                value={formData.filters.isOwner === undefined ? 'all' : formData.filters.isOwner ? 'true' : 'false'}
                onValueChange={(value) => {
                  setFormData({
                    ...formData,
                    filters: {
                      ...formData.filters,
                      isOwner: value === 'all' ? undefined : value === 'true',
                    },
                  });
                }}
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="true">Owners Only</SelectItem>
                  <SelectItem value="false">Non-Owners Only</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-3 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <h3 className="font-medium text-sm flex items-center gap-2 text-blue-900">
                <Activity className="w-4 h-4" />
                Quick Filter Presets
              </h3>
              <p className="text-xs text-blue-700 mb-3">
                Click a preset to quickly apply common filters. You can still customize after applying.
              </p>
              <div className="flex flex-wrap gap-2">
                <Button
                  type="button"
                  ...
                  onClick={() => {
                    setFormData({
                      ...
                      filters: {
                        ...formData.filters,
                        accountCreated: { operator: 'less_than', days: 7 },
                      },
                    });
                  }}
                  className="text-xs"
                ...
                <Button
                  ...
                  onClick={() => {
                    setFormData({
                      ...
                      filters: {
                        ...formData.filters,
                        accountCreated: { operator: 'less_than', days: 14 },
                      },
                    });
                  }}
                  className="text-xs"
                ...
                <Button
                  ...
                  onClick={() => {
                    setFormData({
                      ...
                      filters: {
                        ...formData.filters,
                        accountCreated: { operator: 'less_than', days: 30 },
                      },
                    });
                  }}
                  className="text-xs"
                ...
                <Button
                  ...
                  onClick={() => {
                    setFormData({
                      ...
                      filters: {
                        ...formData.filters,
                        lastLogin: { operator: 'greater_than', days: 30 },
                      },
                    });
                  }}
                  className="text-xs"
                ...
                <Button
                  ...
                  onClick={() => {
                    setFormData({
                      ...
                      filters: {
                        ...formData.filters,
                        lastLogin: { operator: 'less_than', days: 7 },
                      },
                    });
                  }}
                  className="text-xs"
                ...
                <Button
                  ...
                  onClick={() => {
                    setFormData({
                      ...
                      filters: {
                        ...formData.filters,
                        accountCreated: { operator: 'between', daysFrom: 25, daysTo: 30 },
                      },
                    });
                  }}
                  className="text-xs"
                >
                  Trial Ending (25-30 days)
                </Button>
              </div>
            </div>

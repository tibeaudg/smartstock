...
import { AlertCircle, Check, ChevronsUpDown, Plus, Scan, Info, X, Image as ImageIcon, ChevronDown, ChevronUp, Package, Truck } from 'lucide-react';
...
export interface AddProductFormProps {
  formId?: string;
  mode: AddProductMode;
  onModeChange?: (mode: AddProductMode) => void;
  preFilledSKU?: string;
  ...
  onClose?: () => void;
  onShowBulkImporter?: () => void;
  addAnotherRef?: React.MutableRefObject<boolean>;
  hideVariantsInitially?: boolean;
  onStateChange?: (state: AddProductFormState) => void;
}
...
export function AddProductForm({
  ...
  onClose,
  onShowBulkImporter,
  addAnotherRef: externalAddAnotherRef,
  hideVariantsInitially = false,
  onStateChange,
}: AddProductFormProps) {
  const { user } = useAuth();
  ...
  const [showAdditionalInfo, setShowAdditionalInfo] = useState(mode === 'full');
  ...
  const internalAddAnotherRef = useRef(false);
  ...
  const { categories } = useCategoriesFetch({ enabled: !!user && isActive });
  ...
  useEffect(() => {
    if (fromPurchaseOrder && isActive && !editMode) {
      form.setValue('quantityInStock', 0);
      form.setValue('minimumStockLevel', 0);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fromPurchaseOrder, isActive, editMode]);

  useEffect(() => {
    if (editMode && existingProduct && isActive) {
      form.reset({
        name: (existingProduct.name as string) || '',
        description: (existingProduct.description as string) || '',
        categoryId: (existingProduct.category_id as string) || '',
        categoryName: (existingProduct.category_name as string) || '',
        supplierId: (existingProduct.supplier_id as string) || '',
        supplierName: (existingProduct.supplier_name as string) || '',
        quantityInStock: (existingProduct.quantity_in_stock as number) || 0,
        minimumStockLevel: (existingProduct.minimum_stock_level as number) || 10,
        purchasePrice: (existingProduct.purchase_price as number) || 0,
        salePrice: (existingProduct.sale_price as number) || 0,
        location: (existingProduct.location as string) || '',
        sku: (existingProduct.sku as string) || '',
        taxRate: 0,
        taxInclusive: false,
      });
      if (existingProduct.image_url) {
        setImagePreview(existingProduct.image_url as string);
      }
      if (!existingProduct.is_variant && existingProduct.id) {
        supabase
          .from('products')
          .select('*')
          .eq('parent_product_id', existingProduct.id as string)
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [editMode, existingProduct, isActive]);
  ...
  }, [loading, duplicateName, hasVariants, isOverProductLimit, nameValue, isQuickMode, onStateChange]);

  // On mount: restore draft if one exists, otherwise start fresh
  useEffect(() => {
    if (!activeBranch?.branch_id || !isActive || editMode) return;
    ...
    setUploadedImages([]);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeBranch?.branch_id, isQuickMode, isActive, editMode]);
  ...
  useEffect(() => {
    ...
    track('product_form_opened', editMode ? 'Product Form Opened (Edit)' : 'Product Form Opened', {
      method: isQuickMode ? 'quick' : 'modal',
      editMode,
    });
    return () => {
      if (!submittedRef.current) {
        track('product_form_abandoned', 'Product Form Abandoned', {
          method: isQuickMode ? 'quick' : 'modal',
          editMode,
        });
      }
  ...
  useEffect(() => {
    ...
    const checkDuplicate = async (name: string) => {
      name = name.trim();
      if (editMode || !name || !activeBranch?.branch_id || hasVariants) {
  ...
  }, [form, activeBranch, hasVariants, editMode]);
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const supplierId = validatedData.supplierName
        ? await resolveSupplierId(validatedData.supplierName)
      ...
      if (!hasVariants) {
        if (editMode && existingProduct?.id) {
          ...
          const { error: transactionError } = await supabase.from('stock_transactions').insert(stockTransactionData);
          if (transactionError) {
      ...
      if (addAnotherRef.current) {
        ...
        form.setValue('quantityInStock', isQuickMode ? 1 : 0);
        setTimeout(() => form.setFocus('name'), 0);
      } else {
        onProductAdded?.(
          insertedProduct
            ? {
                id: insertedProduct.id as string,
                name: insertedProduct.name as string,
                purchase_price: insertedProduct.purchase_price as number | undefined,
                unit_price: insertedProduct.unit_price as number | undefined,
              }
            : undefined
        );
        onClose?.();
  ...
  if (!user || branchLoading || !isActive) {
    return null;
  }
    ...
    <>
        ...
        {isOverProductLimit && (
          <div className="flex-shrink-0 flex items-center gap-3 bg-amber-50 border-b border-amber-200 px-6 py-3">
            ...
            <Form {...form}>
              <form id={formId} onSubmit={form.handleSubmit(handleSubmit, (errors) => {
                  ...
                  <CollapsibleContent className="space-y-4 mt-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-gray-900">Supplier <span className="text-gray-400 font-normal">(optional)</span></FormLabel>

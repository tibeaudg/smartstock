...
import { UpgradeOrPayModal } from './UpgradeOrPayModal';
...
export const BranchPickerDropdown: React.FC<BranchPickerDropdownProps> = ({
  ...
  const { canAddBranch, isOverBranchLimit } = useSubscription();
  const {
    branches,
    activeBranch,
    setActiveBranch,
  ...
  const isAccountRestricted = userProfile?.blocked || !canAddBranch;

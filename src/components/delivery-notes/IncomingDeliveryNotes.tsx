...
export const IncomingDeliveryNotes: React.FC = () => {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const [isCreateDialogOpen, setIsCreateDialogOpen] = useState(false);
  const [selectedNote, setSelectedNote] = useState<DeliveryNote | null>(null);

  // Fetch incoming delivery notes
  const { data: deliveryNotes = [], isLoading } = useQuery<DeliveryNote[]>({
    queryKey: ['delivery-notes', 'incoming'],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: 'De inkomende leveringsbon is succesvol aangemaakt.',
      });
    },
    onError: (error) => {
      toast({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Inkomende Leveringsbonnen</h2>
          <p className="text-gray-600 mt-1">Beheer leveringsbonnen van leveranciers</p>
        </div>
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>Nieuwe Inkomende Leveringsbon</DialogTitle>
            </DialogHeader>
            <form action={handleCreateNote} className="space-y-4">
              ...
              <div>
                <Label htmlFor="customer_name">Leverancier</Label>
                <Input
                  id="customer_name"
                  name="customer_name"
        ...
        {deliveryNotes.length === 0 ? (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-gray-600 mb-4">Je hebt nog geen inkomende leveringsbonnen aangemaakt.</p>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{note.customer_name || 'Geen leverancier'}</span>
                      </div>
                      <div className="flex items-center gap-2">
